---
title: "關於幾個將 Win App 容器化的資訊"
date: 2021-10-24T04:36:32Z
draft: false
tags: ["docker"]
---

## Youtube

- [Debug WPF and Winforms .NET in Linux on a Remote Machine](https://youtu.be/mditvqEDLyQ)
- [Delivering Desktop Apps in Containers](https://youtu.be/L4nqky8qGm8)
- [Containerize WINDOWS desktop apps with LINUX containers using WINE.](https://youtu.be/3juYY9dzz3w)
- [Desktop Apps in Docker Containers](https://youtu.be/YbXDJJE5zsc)

## Docker

- [scottyhardy/docker-wine](https://hub.docker.com/r/scottyhardy/docker-wine/)
- [wine-x11-novnc-docker](https://github.com/solarkennedy/wine-x11-novnc-docker)
- [Run A windows application with docker](https://github.com/sykuang/docker-wine-x11-novnc)
